...
public final class RenderWirelessEndpoint {
	...
	public static void drawGlow(State state) {
		...
		switch (state) {
			case ERROR:
				tas = Minecraft.getMinecraft().getTextureMapBlocks().getAtlasSprite(CoCore.MODID+":blocks/wireless_endpoint_error");
				break;
			case LINKED:
				tas = Minecraft.getMinecraft().getTextureMapBlocks().getAtlasSprite(CoCore.MODID+":blocks/wireless_endpoint_linked");
				break;
			default:
				break;
		}
