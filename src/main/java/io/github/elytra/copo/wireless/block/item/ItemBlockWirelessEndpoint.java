package io.github.elytra.copo.wireless.block.item;
...
public class ItemBlockWirelessEndpoint extends ItemBlock {
	...
	public void addInformation(ItemStack stack, EntityPlayer playerIn, List<String> tooltip, boolean advanced) {
		tooltip.add(I18n.format("tooltip."+CoCore.MODID+".rf_usage", 24));
	}
	
	@Override
	public String getUnlocalizedName(ItemStack stack) {
		return stack.getItemDamage() == 0 ? 
				"tile."+CoCore.MODID+".wireless_receiver" :
				"tile."+CoCore.MODID+".wireless_transmitter";
	}
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (stack.getMetadata() == 0) {
			...
			if (te instanceof TileEntityWirelessTransmitter) {
				...
				if (world.isRemote) {
					player.addChatMessage(new TextComponentTranslation("msg."+CoCore.MODID+".receiver_linked"));
				}
				return EnumActionResult.FAIL;
			}
			if (!stack.hasTagCompound() || !stack.getTagCompound().hasKey("TransmitterUUIDMost")) {
				if (world.isRemote) {
					player.addChatMessage(new TextComponentTranslation("msg."+CoCore.MODID+".receiver_unlinked"));
				}
				return EnumActionResult.FAIL;
			}
		}
