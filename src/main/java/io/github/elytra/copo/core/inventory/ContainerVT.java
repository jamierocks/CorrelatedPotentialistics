package io.github.elytra.copo.core.inventory;
...
public class ContainerVT extends Container {
	...
	public class SlotVirtual extends Slot {
		...
		public void putStack(ItemStack stack) {
			if (world.isRemote) {
				...
				if (stack != null) {
					CoCore.log.warn("putStack was called on a virtual slot", new RuntimeException()
						.fillInStackTrace());
	...
	public ItemStack removeItemsFromNetwork(ItemStack prototype, int amount) {
		...
		stack.stackSize = 0;
		stack = vt.getController().removeFromNetwork(stack, amount).getThing();
		return stack;
	}
				...
				if (cur > 127 || cur < -128) {
					for (IContainerListener ic : listeners) {
						if (ic instanceof EntityPlayerMP) {
							...
							if (cur > 127) {
								CoCore.inst.network.sendTo(new SetSlotSizeMessage(windowId, i, cur), p);
							}
						}
					}
				}
	...
	public void addListener(IContainerListener listener) {
		...
		if (listener instanceof EntityPlayerMP) {
			CoCore.inst.network.sendTo(new SetSearchQueryMessage(windowId, searchQuery), (EntityPlayerMP)listener);
		}
	}
