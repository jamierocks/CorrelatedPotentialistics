...
import io.github.elytra.copo.core.inventory.ContainerVT.CraftingAmount;
import io.github.elytra.copo.core.inventory.ContainerVT.CraftingTarget;
import io.github.elytra.copo.core.inventory.ContainerVT.SlotVirtual;
import io.github.elytra.copo.core.inventory.ContainerVT.SortMode;
...
public class GuiVT extends GuiContainer {
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		fontRendererObj.drawString(I18n.format("gui."+CoCore.MODID+".vt"), 8, 6, 0x404040);
		...
		if (sortMode.isMouseOver()) {
			drawHoveringText(Lists.newArrayList(
					I18n.format("tooltip."+CoCore.MODID+".sortmode"),
					"\u00A77"+I18n.format("tooltip."+CoCore.MODID+".sortmode."+container.sortMode.lowerName)
				), mouseX, mouseY);
		}
		if (sortDirection.isMouseOver()) {
			String str = (container.sortAscending ? "ascending" : "descending");
			drawHoveringText(Lists.newArrayList(
					I18n.format("tooltip."+CoCore.MODID+".sortdirection"),
					"\u00A77"+I18n.format("tooltip."+CoCore.MODID+".sortdirection."+str)
				), mouseX, mouseY);
		}
		if (craftingAmount.isMouseOver()) {
			drawHoveringText(Lists.newArrayList(
					I18n.format("tooltip."+CoCore.MODID+".crafting_amount"),
					"\u00A77"+I18n.format("tooltip."+CoCore.MODID+".crafting.only_shift_click"),
					"\u00A77"+I18n.format("tooltip."+CoCore.MODID+".crafting_amount."+container.craftingAmount.lowerName)
				), mouseX, mouseY);
		}
		if (craftingTarget.isMouseOver()) {
			drawHoveringText(Lists.newArrayList(
					I18n.format("tooltip."+CoCore.MODID+".crafting_target"),
					"\u00A77"+I18n.format("tooltip."+CoCore.MODID+".crafting.only_shift_click"),
					"\u00A77"+I18n.format("tooltip."+CoCore.MODID+".crafting_target."+container.craftingTarget.lowerName)
				), mouseX, mouseY);
		}
		if (clearGrid.isMouseOver()) {
			drawHoveringText(Lists.newArrayList(I18n.format("tooltip."+CoCore.MODID+".clear_crafting_grid")), mouseX, mouseY);
		}
		GlStateManager.popMatrix();
	...
	public void updateScreen() {
		...
		if (ticksSinceLastQueryChange == 4) {
			CoCore.inst.network.sendToServer(new SetSearchQueryMessage(container.windowId, lastSearchQuery));
		}
	}
