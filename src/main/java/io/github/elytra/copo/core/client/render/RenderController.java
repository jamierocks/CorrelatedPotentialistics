...
public class RenderController extends TileEntitySpecialRenderer<TileEntityController> {
	...
	public void renderTileEntityAt(TileEntityController te, double x, double y, double z, float partialTicks, int destroyStage) {
		...
		if (bs.getBlock() != CoCore.controller) return;
		...
		switch (state) {
			case BOOTING:
				tex = CoCore.MODID+":blocks/controller_booting";
				break;
			case ERROR:
				tex = CoCore.MODID+":blocks/controller_error";
				break;
			case OFF:
				tex = CoCore.MODID+":blocks/controller_off";
				break;
			case POWERED:
				tex = CoCore.MODID+":blocks/controller";
				break;
			default:
				return;
		}
