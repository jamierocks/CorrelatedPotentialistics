package io.github.elytra.copo.core.client;
...
import io.github.elytra.copo.core.CoreProxy;
import io.github.elytra.copo.core.client.render.RenderController;
import io.github.elytra.copo.core.client.render.RenderDriveBay;
import io.github.elytra.copo.core.client.render.RenderVT;
...
public class CoreClientProxy extends CoreProxy {
	...
	public void preInit() {
		...
		for (String s : ItemMisc.items) {
			ModelLoader.setCustomModelResourceLocation(CoCore.misc, idx++, new ModelResourceLocation(new ResourceLocation(CoCore.MODID, s), "inventory"));
		}
	}
	@Override
	...
	public void registerItemModel(Item item, int variants) {
		...
		if (variants < -1) {
			...
			loc = new ResourceLocation(CoCore.MODID, "tesrstack");
		}
		if (variants == -1) {
			List<ItemStack> li = Lists.newArrayList();
			item.getSubItems(item, CoCore.creativeTab, li);
			for (ItemStack is : li) {
