...
public class BlockVT extends Block {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (!player.isSneaking()) {
				...
				if (tenm.hasController()) {
					if (!world.isRemote) {
						switch (world.getBlockState(tenm.getController().getPos()).getValue(BlockController.state)) {
							case BOOTING:
								player.addChatMessage(new TextComponentTranslation("msg."+CoCore.MODID+".vt_booting"));
								break;
							case ERROR:
								player.addChatMessage(new TextComponentTranslation("msg."+CoCore.MODID+".vt_error"));
								break;
							case OFF:
								player.addChatMessage(new TextComponentTranslation("msg."+CoCore.MODID+".vt_no_power"));
								break;
							case POWERED:
								player.openGui(CoCore.inst, 0, world, pos.getX(), pos.getY(), pos.getZ());
								break;
							default:
								break;
			...
			if (!world.isRemote) {
				player.addChatMessage(new TextComponentTranslation("msg."+CoCore.MODID+".vt_no_controller"));
			}
			return true;
		}
